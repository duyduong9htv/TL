      program ram
c
c     Double precision for high frequency problems
c
...
c
     ...
     >   tlg_alog(mz)
      real*8 k0,rb(mr),zb(mr),cw(mz),cb(mz),rhob(mz),attn(mz),alpw(mz),
...
c
     ...
     >   attn,alpw,alpb,ksqw,ksqb)
      ...
      real*8 k0,cw(mz),cb(mz),rhob(mz),attn(mz),alpw(mz),alpb(mz),
     >   ksqw(mz)
c
...
c
     ...
     >   r3(mz,mp),s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp)
      real*8 k0,rhob(mz),f1(mz),f2(mz),f3(mz),alpw(mz),alpb(mz),
     >   ksqw(mz)
c
      a1=k0**2/6.0
      a2=2.0*k0**2/3.0
...
c
      ...
      implicit real*8 (a-h,o-z)
      complex*16 u(mz),v(mz),r1(mz,mp),r2(mz,mp),r3(mz,mp),s1(mz,mp),
     >   s2(mz,mp),s3(mz,mp)
      eps=1.0d-30
c
      do 6 j=1,np
...
c
     ...
     >   s1(mz,mp),s2(mz,mp),s3(mz,mp),pd1(mp),pd2(mp)
      real*8 k0,rb(mr),zb(mr),attn(mz),cb(mz),rhob(mz),cw(mz),ksqw(mz),
...
c
     ...
     >    tlg_alog)
      implicit real*8 (a-h,o-z)
      complex*16 ur,u(mz),tlg_alog(mz),ci
      real*8 f3(mz),tlg(mz)
c
      ci=cmplx(0.0,1.0)
c
...
c
      ...
      tlg_alog(j)= ur / dsqrt(r + eps)
    1 continue
      write(3)(tlg(j),j=1,lz)
      write(23)(dble(tlg_alog(j)),j=1,lz)
      write(23)(dimag(tlg_alog(j)),j=1,lz)
      end if
c
      return
      end
...
c
      implicit real*8 (a-h,o-z)
      complex*16 ci,g0,tfact,rot0,rot1,rot2,pd1(mp),pd2(mp)
      real*8 k0
      pi=4.0*atan(1.0)
      ci=cmplx(0.0,1.0)
      tfact=cdexp(-ci*theta*pi/360.0)
      den=float(2*np+1)
      rot0=1.0
c
      do 1 j=1,np
