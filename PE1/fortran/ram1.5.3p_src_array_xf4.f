c     2005 05 23
c     modified to read source array position from source_spacing.in
c     format of source_spacing.in:
c        number of source elements
c        relative position of the elements from the first source element
c        the depth of the first source element is read from ram.in
c     
c
c     2005 05 05
c     modified to output complex pressure in tl.line
c
      program ram
c
c     ******************************************************************
...
c
      ...
      real k0,ksqw
c    modified by sunwoong
      complex tlg_alog
c    modified by sunwoong end
c
c     mr=bathymetry points, mz=depth grid, mp=pade terms.
c
      parameter (mr=6000,mz=22000,mp=10)
...
c    modified by sunwoong
      dimension tlg_alog(mz)
c    modified by sunwoong end
c
      open(unit=1,status='old',file='ram.in')
c    modified by sunwoong
c    reads source spacing from source_spacing.in
      open(unit=11,status='old',file='source_spacing_xf4.in')
c    modified by sunwoong end
      open(unit=2,status='unknown',file='tl.line')
      open(unit=3,status='unknown',file='tl.grid',form='unformatted')
c     modified by sunwoong
      open(unit=1,status='old',file='ram.in')
      open(unit=23,status='unknown',file='g_grid.dat',
     >   form='unformatted') 
c     modified by sunwoong end
c
c    modified by sunwoong
c      call setup(mr,mz,nz,mp,np,ns,mdr,ndr,ndz,iz,nzplt,lz,ib,ir,dir,dr,
c     >   dz,pi,eta,eps,omega,rmax,c0,k0,ci,r,rp,rs,rb,zb,cw,cb,rhob,
c     >   attn,alpw,alpb,ksq,ksqw,ksqb,f1,f2,f3,u,v,r1,r2,r3,s1,s2,s3,
c     >   pd1,pd2,tlg)
...
c
c     modified by sunwoong
c      subroutine setup(mr,mz,nz,mp,np,ns,mdr,ndr,ndz,iz,nzplt,lz,ib,ir,
c     >   dir,dr,dz,pi,eta,eps,omega,rmax,c0,k0,ci,r,rp,rs,rb,zb,cw,cb,
...
c     modified by sunwoong to account for source array
c     maximum number of source elements
      parameter (max_source_element=50)
      real source_depths(max_source_element)
      real temp_spacing
      integer number_of_source_elements
      complex u_save(mz)
      real zs_element
c
      source_depths(1) = zs
c
      read(11, *) number_of_source_elements
      write(*,*)'   number of source elements = ',
     >                 number_of_source_elements
      do 55 i=1,number_of_source_elements-1
      read(11, *) temp_spacing
      source_depths(i+1) = source_depths(i) + temp_spacing
   55 continue
      close(11)
      do 66 i = 1,number_of_source_elements
      write(*,*)'   source depths = ',source_depths(i)
   66 continue
c
c
      zs_element = source_depths(1)
c
      i_source_number = 1
    ...
    1 u_save(i_index) = 0
      i_index = i_index + 1
c     if (i_index .le. max_source_element) go to 1
      if (i_index .le. mz) go to 1

    2 si=1.0+zs_element/dz
      do 10 j=1,mp
      r3(1,j)=0.0
      r1(nz+2,j)=0.0
   10 continue
   ...
   11 continue
c
c     modified by sunwoong to account for source array end
c
...
c     modified by sunwoong to account for source array
      i_index = 1
    5 u_save(i_index) = u_save(i_index) + u(i_index)
      i_index = i_index + 1
      if (i_index .le. mz) go to 5
c
      write(*,*)'   i_source_number = ',i_source_number
      i_source_number = i_source_number + 1
      zs_element = source_depths(i_source_number)
      if (i_source_number .le. number_of_source_elements) go to 2
c
      i_index = 1
    6 u(i_index) = u_save(i_index) / number_of_source_elements
      i_index = i_index + 1
      if (i_index .le. mz) go to 6
c     modified by sunwoong to account for source array end
c
      return
      end
...
c
c     modified by sunwoong 
c      subroutine outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,tlg)
...
c     modified by sunwoong 
      complex tlg_alog(mz)
      complex ci 
c     Modified by Purnima
      complex afact
      real k0

      ci=cmplx(0.0,1.0)
c     Modified by Purnima
      afact=cexp(ci*k0*r-ci*pi/4.0)
...
c     modified by sunwoong 
c     first column: r
c     second column: transmission loss in dB
c     third column: real part of the Green's function
c     fourth column: imag part of the Green's function
      write(2,*)r,tl, real(ur)/sqrt(r+eps), imag(ur)/sqrt(r+eps)
c     modified by sunwoong  end
c
      mdr=mdr+1
      if(mdr.eq.ndr)then
...
c     modified by sunwoong 
cc      write(*,*)'  ur / sqrt(r + eps) = ', ur / sqrt(r + eps)
c      tlg_alog(j)= ur / sqrt(r + eps)
c     Modified by Purnima
      tlg_alog(j)= ur*afact / sqrt(r + eps)
c     modified by sunwoong end
    1 continue
      write(3)(tlg(j),j=1,lz)
c     modified by sunwoong 
      write(23)(real(tlg_alog(j)),j=1,lz)
      write(23)(real(-ci * tlg_alog(j)),j=1,lz)
c     modified by sunwoong end
      end if
c
      return
