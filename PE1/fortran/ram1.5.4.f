c     2006 05 10
c     modified to output only mag. squared pressure and nr
c
c     2005 05 05
...
c     open(unit=2,status='unknown',file='tl.line')
      open(unit=3,status='unknown',file='psq.grid',
     >    form='unformatted')
      open(unit=21,status='unknown',file='nr.dat')
c    modified by sunwoong end
c
c    modified by sunwoong
...
c      call outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,tlg)
      call outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,
     >   psq, nr)
c     modified by sunwoong end
...
c     modified by sunwoong end
     ...
     >   alpb(mz),f1(mz),f2(mz),f3(mz),ksqw(mz)
c     modified by sunwoong
      real psq(mz)
c     modified by sunwoong end
c
...
c      subroutine outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,tlg)
      subroutine outpt(mz,mdr,ndr,ndz,iz,nzplt,lz,ir,dir,eps,r,f3,u,
     >    psq, nr)
c     modified by sunwoong end
      complex ur,u(mz)
      real f3(mz)
...
c     modified by sunwoong 
c     write(2,*)r,tl, real(ur) / sqrt(r+eps), imag(ur) / sqrt(r+eps)
c     modified by sunwoong  end
c
      mdr=mdr+1
      if(mdr.eq.ndr)then
...
c      write(*,*)'  ur / sqrt(r + eps) = ', ur / sqrt(r + eps)
      psq(j)= cabs(ur)**2 / (r + eps)
c     modified by sunwoong end
    1 continue
c     modified by sunwoong 
      write(3)(psq(j),j=1,lz)
c     modified by sunwoong end
      end if
c
      return
